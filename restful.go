...
func (r *Request) GenerateWallet(w http.ResponseWriter, re *http.Request) {
	...
	// -----------------------------------------JSON 해독 끝 -------------------------
	// ------------------------- RPC 서버 연결 ---------------------
	Client, err := rpc.Dial("tcp", "192.168.10.146:9000")
	if err != nil {
		fmt.Println(err)
		return
	}
...
func (r *Request) CheckAddress(w http.ResponseWriter, re *http.Request) {
	...
	if response.Check {
		...
		res, err := http.Post("http://192.168.10.99:5000/refTx", "application/json", bytes.NewBuffer(json_data))
		if err != nil {
			fmt.Println(err)
			return
		} // 받아온 Txs를 돌려줌
...
func (r *Request) RegisterCareer(w http.ResponseWriter, re *http.Request) {
	Res, err := http.Post("http://192.168.10.99:5000/newBlk", "application/json", re.Body)
	if err != nil {
		fmt.Println(err)
		return
	}
...
func (r *Request) findDetail(w http.ResponseWriter, re *http.Request) {
	Res, err := http.Post("http://192.168.10.99:5000/detailTx", "application/json", re.Body)
	if err != nil {
		fmt.Println(err)
		return
	}
